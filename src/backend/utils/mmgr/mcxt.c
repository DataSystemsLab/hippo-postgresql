...
palloc(Size size)
{
	...
	if (!AllocSizeIsValid(size))
		elog(ERROR, "This palloc, invalid memory alloc request size %zu", size);
...
palloc0(Size size)
{
	...
	if (!AllocSizeIsValid(size))
		elog(ERROR, "This is palloc0, invalid memory alloc request size %zu", size);
...
palloc_extended(Size size, int flags)
{
	...
	if (((flags & MCXT_ALLOC_HUGE) != 0 && !AllocHugeSizeIsValid(size)) ||
		((flags & MCXT_ALLOC_HUGE) == 0 && !AllocSizeIsValid(size)))
		elog(ERROR, "This is palloc _extend,invalid memory alloc request size %zu", size);
...
repalloc(void *pointer, Size size)
{
	...
	if (!AllocSizeIsValid(size))
		elog(ERROR, "This is repalloc, invalid memory alloc request size %zu", size);
